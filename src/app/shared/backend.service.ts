...
export class BackendService {
  ...
  public async getSensorenDatenByPage(page:number, sensorsPerPage:number){
    const sensorenDataResponse = await firstValueFrom(this.http.get<SensorendataResponse[]>(`http://localhost:5000/sensorsData?_page=${page}&_limit=${sensorsPerPage}`, { observe: 'response' }));
const sensorenData: Sensorendata[]= sensorenDataResponse.body!.map(data => {
  const sensor: Sensor = this.sensoren.filter(sensor => sensor.id == data.sensorId)[0];
  return { ...data, sensor }
});
this.storeService.sensorenDaten = sensorenData;
return Number(sensorenDataResponse.headers.get('X-Total-Count'));
  }
